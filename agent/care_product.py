"""ClinicaLens C-end v2 product projections and deterministic safety content.

This module contains only the explicitly fictional sandbox case and curated
decision-support copy.  It does not call an LLM and never creates a prescription.
"""
...
def sample_raw_case_document() -> Dict[str, Any]:
    sections = [
        {"id": "identity", "title": "患者信息", "text": "周予安，男，46 岁。沙箱病历号 SBX-20260902-001。所有身份信息均为虚构。"},
        {"id": "complaint", "title": "主诉与现病史", "text": "2 周来乏力、双膝及腕关节酸痛；3 天来出现血丝痰和活动后呼吸困难。无大咯血，当前无静息呼吸困难、胸痛或意识改变。症状较前加重，因此就诊。"},
        {"id": "vitals", "title": "生命体征", "text": "体温 37.4℃，心率 102 次/分，呼吸 22 次/分，血压 148/92 mmHg，静息指氧 94%（空气）。"},
        {"id": "history", "title": "既往史、手术史与家族史", "text": "高血压 3 年；否认糖尿病及既往慢性肾病。2012 年行阑尾切除。无明确自身免疫病或遗传性肾病家族史。"},
        {"id": "medication", "title": "用药史与过敏史", "text": "长期服用氨氯地平 5 mg 每日一次；近一个月因关节痛间断服用布洛芬 200 mg。无已知药物、食物及造影剂过敏。"},
        {"id": "social", "title": "个人史与暴露史", "text": "既往约 10 包年吸烟史，2 年前戒烟；偶尔饮酒。职业为室内设计项目经理，偶有装修粉尘暴露，否认石棉和禽类长期接触。"},
        {"id": "basic", "title": "基础检验与影像", "text": "尿红细胞 50 个/HPF（参考值 0–3），尿蛋白 2+，尿沉渣见畸形红细胞；肌酐 220 μmol/L（参考值 44–133），eGFR 29 ml/min/1.73m²。胸部 CT 示双肺磨玻璃影，影像考虑弥漫性肺泡出血。"},
        {"id": "serology", "title": "免疫学与感染筛查", "text": "MPO-ANCA 86 RU/ml 阳性，PR3-ANCA 阴性，抗 GBM 抗体阴性，补体 C3/C4 正常。血培养及痰病原学筛查阴性；现有结果不能排除全部感染。"},
        {"id": "biopsy", "title": "肾活检", "text": "肾活检示少免疫沉积性坏死性新月体性肾小球肾炎，支持 ANCA 相关小血管炎。"},
        {"id": "doctor", "title": "医生诊断与出院记录", "text": "沙箱医生综合肺泡出血、肾小球肾炎、MPO-ANCA 与肾活检结果，确认显微镜下多血管炎。治疗与剂量均由沙箱医生记录生成，并安排风湿免疫科、肾内科联合复诊。"},
    ]
    return {
        "id": "fictional-case-mpa-001",
        "title": "周予安完整虚构病例原文",
        "is_fictional": True,
        "source": "ClinicaLens 产品沙箱",
        "created_on": "2026-09-02",
        "sections": sections,
        "full_text": "\n\n".join(f"【{item['title']}】\n{item['text']}" for item in sections),
        "notice": "病例为完整虚构内容，可用于功能体验，不代表真实诊疗记录。",
    }
...
def treatment_reference() -> Dict[str, Any]:
    return {
        ...
        "limitations": "本层只介绍常见路径类别，不包含个体剂量、处方、停药或换药建议，不能创建用药任务。",
    }
...
def assessment_version(journey: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "limitations": ["这是 AI 辅助判断，不是医生确诊。", "诊断版本只使用当时已经同步并确认的证据。", "治疗参考不包含个体处方或剂量。"],
    }
...
def hydrate_journey(journey: Dict[str, Any]) -> Dict[str, Any]:
    ...
    upgraded["schema_version"] = "care-journey.v2"
...
__all__ = ["all_sample_records", "answer_consultation", "assessment_version", "build_evidence", "hydrate_journey", "medication_education", "public_sample_journey", "quick_questions", "sample_clinical_history", "sample_patient_profile", "sample_raw_case_document", "sample_record_batches", "treatment_reference"]
